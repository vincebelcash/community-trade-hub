      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
          <div className="space-y-1">
            <h1 className="text-4xl font-bold tracking-tight uppercase italic leading-none">HellOOpass <span className="text-blue-600">Registry</span></h1>
            <p className="text-lg text-slate-500 font-medium italic">Manage your community participation and trading activity.</p>
          </div>
          <div className="flex items-center space-x-3">
            <Button variant="outline" className="rounded-xl font-bold uppercase tracking-widest text-[9px] border border-slate-200 h-14 px-6 hover:bg-slate-100 transition-all">Report</Button>
            <Button onClick={() => setIsTradeModalOpen(true)} className="rounded-xl font-bold uppercase tracking-widest text-[9px] bg-blue-600 hover:bg-blue-700 text-white shadow-xl h-14 px-8 transition-all transform hover:scale-105">
              Quick Trade
            </Button>
          </div>
        </div>
        ...
        <Tabs defaultValue="overview" className="space-y-8">
          <TabsList className="bg-white border border-slate-100 w-full md:w-auto overflow-x-auto justify-start h-auto p-1.5 rounded-2xl shadow-sm">
            {['Overview', 'Networks', 'Market', 'Wallet', 'Settings'].map((tab) => (
              <TabsTrigger 
                key={tab.toLowerCase()} 
                value={tab.toLowerCase()} 
                className="px-8 py-3.5 font-bold uppercase tracking-widest text-[9px] data-[state=active]:bg-slate-900 data-[state=active]:text-white transition-all rounded-xl"
              >
                {tab}
              </TabsTrigger>
            ))}
