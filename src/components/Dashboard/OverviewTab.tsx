    ...
    >
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm">Manage your community assets and growth pools.</p>
        </div>
        <div className="flex items-center gap-2 bg-slate-900 border border-slate-800 px-4 py-2 rounded-xl">
          ...
          <span className="text-xs font-medium text-slate-300 tracking-wide uppercase">Biometric Verified</span>
        </div>
      </header>
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <ActionButton icon={<ArrowUpRight size={24} />} label="Send" onClick={onSend} color="#008752" />
          <ActionButton icon={<ArrowDownLeft size={24} />} label="Receive" onClick={onReceive} color="#FFD700" />
          <ActionButton icon={<QrCode size={24} />} label="Pay Merchant" onClick={() => toast.info('Scan to pay interface coming soon')} color="#EF3340" />
          <ActionButton icon={<CreditCard size={24} />} label="Cards" onClick={() => toast.info('Virtual cards ready')} color="#008752" />
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
         <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-sm text-slate-400">Total Pooled Capital</span>
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm text-slate-400">Avg. Community ROI</span>
            ...
            <button onClick={onEnterMarket} className="w-full mt-4 py-2 bg-slate-800 text-slate-300 rounded-lg text-sm font-medium hover:bg-slate-700 transition-colors flex items-center justify-center gap-2 uppercase tracking-widest text-[10px] font-black">
              Enter Market <ChevronRight size={14} />
            </button>
          </div>
        </div>
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl overflow-hidden">
          <div className="p-6 border-b border-slate-800 flex items-center justify-between">
            ...
            <button className="text-[#008752] text-xs font-black uppercase hover:underline">View All</button>
          </div>
          <div className="divide-y divide-slate-800">
            <ActivityRow 
              title="Organic Farm Box" 
              sub="Trade Transaction" 
              amount="-\u20ac15.00" 
              bonus="+1.50 Bonus"
              date="Today"
              accentColor="#008752"
