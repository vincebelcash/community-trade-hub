...
export function SendModal({ isOpen, onClose }: ModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Send Capital">
      <div className="space-y-6">
        <div className="space-y-2">
          <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Recipient Member</label>
        ...
        <div className="space-y-2">
          <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Amount (EUR)</label>
          <input 
            type="number" 
            placeholder="0.00"
            value={amount}
        ...
        >
            {isProcessing ? 'Verifying Biometrics...' : 'Send Capital'}
...
export function ReceiveModal({ isOpen, onClose }: ModalProps) {
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="My Merchant QR">
      <div className="flex flex-col items-center gap-8 py-4">
        ...
        <div className="w-full p-4 bg-[#FFD700]/10 border border-[#FFD700]/20 rounded-xl flex items-center gap-3">
            ...
            <p className="text-xs text-slate-300">Scanning this QR will initiate a biometric-verified trade within the network.</p>
        </div>
      </div>
    </Modal>
  );
...
export function JoinNetworkModal({ isOpen, onClose }: ModalProps) {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Join Community Network">
            <div className="space-y-6">
                <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Invite Code</label>
                    <input 
                        type="text" 
                ...
                <p className="text-xs text-slate-500 leading-relaxed">
                    Invite codes are issued by community administrators. Once joined, your zero-cost wallet will be initialized for this network.
                </p>
                <button 
                ...
                >
                    {isJoining ? 'Verifying Invite...' : 'Join Economy'}
                </button>
            </div>
        </Modal>
    );
